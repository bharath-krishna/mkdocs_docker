...
def test_serve(test_name, call_api):
    ...
    ports = {'8000/tcp': 8000}
    try:
        cnt = client.containers.run("mkdocs_image", "serve", remove=True, volumes=PWDIR_VOL, ports=ports, detach=True)
...
def test_run(test_name, call_api):
    ...
    ports = {'8000/tcp': 8000}
    try:
        cnt = client.containers.run("mkdocs_image", "run", remove=True, volumes=PWDIR_VOL, ports=ports, detach=True)
